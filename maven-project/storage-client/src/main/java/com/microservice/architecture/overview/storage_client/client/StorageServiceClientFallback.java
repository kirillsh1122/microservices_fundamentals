...
public class StorageServiceClientFallback implements StorageServiceClient {
    ...
    @Override
    public ResponseEntity<StorageEntryIDDTO> createStorageEntity(StorageEntryDTO storageRequestBody) {
        return ResponseEntity.ok(new StorageEntryIDDTO(4444L));
    }

    @Override
    public ResponseEntity<List<StorageEntryIDDTO>> deleteStorageEntriesByIds(String Ids) {
        return ResponseEntity.ok(List.of(new StorageEntryIDDTO(5555L)));
    }
